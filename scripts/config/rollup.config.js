  ...
  output: [
    {
      file: getOutputFileName(
          resolve(PATHS.bundles, 'sysweb3.esm.js'),
          ifProduction()
      ),
      format: 'es',
      sourcemap: true,
...
const UMDconfig = {
  ...
  //context: 'window',
  input: resolve(PATHS.entry.esm, 'index.js'),
  //external: Object.keys(pkg.dependencies || {}).filter(key => /^sysweb3-/.test(key)),
  external: [ 'fs' ],
  output: {
    file: getOutputFileName(
      resolve(PATHS.bundles, 'sysweb3.umd.js'),
      ifProduction()
    ),
    format: 'umd',
    name: LIB_NAME,
    ...
    //globals: Object.assign({}, ...Object.keys(pkg.dependencies || {}).filter(key => /^sysweb3-/.test(key)).map(key => ({[key]: "sysweb3"})))
  },
  plugins: removeEmpty(
    /** @type {Plugin[]} */ ([...plugins, ifProduction(terser())])
  ),
...
const IIFEconfig = {
  ...
  plugins: removeEmpty(([...plugins, ifProduction(terser())])
  ),
}
