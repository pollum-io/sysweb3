...
describe('Key Store', () => {
  ...
  it('IsValid DAG address', async () => {
    ...
    const result = keyStore.validatesysAddress(testData.SYS_ADDRESS);
  ...
  it('IsInvalid DAG address', async () => {
    ...
    //Empty
    const result = keyStore.validatesysAddress('');
    expect(result).to.equal(false);

    //Bad length
    const result0 = keyStore.validatesysAddress('DAG2itmeekZLUS4vxCDhe9safyE6wFQ94EaczotNn');
    expect(result0).to.equal(false);

    //Bad prefix
    const result1 = keyStore.validatesysAddress('DOG2itmeekZLUS4vxCDhe9safyE6wFQ94EaczotN');
    expect(result1).to.equal(false);

    //Bad Parity
    const result2 = keyStore.validatesysAddress('DAGJitmeekZLUS4vxCDhe9safyE6wFQ94EaczotN');
    expect(result2).to.equal(false);

    //Bad Base58 Match
    const result3 = keyStore.validatesysAddress('DAG20itmeekZLUS4vxCDhe9safyE6wFQ94EaczotN');
  ...
  it('DAG address from Public',  () => {
    ...
    const result = keyStore.getsysAddressFromPublicKey(testData.PUBLIC_KEY);
    expect(result).to.equal(testData.SYS_ADDRESS);
  });
