...
describe('Key Store', () => {
  ...
  it('IsValid SYS address', async () => {

    const result = SyscoinWeb3.keyStore.validatesysAddress(testData.SYS_ADDRESS);
  ...
  it('Public key from Private', () => {
    ...
    const result = SyscoinWeb3.keyStore.getPublicKeyFromPrivate(testData.PRIVATE_KEY);
  ...
  it('Compact Public key from Private', () => {
    ...
    const result = SyscoinWeb3.keyStore.getPublicKeyFromPrivate(testData.PRIVATE_KEY, true);
    chai.expect(result).to.equal(testData.COMPACT_PUBLIC_KEY);

  });

  it('SYS address from Public',  () => {

    const result = SyscoinWeb3.keyStore.getsysAddressFromPublicKey(testData.PUBLIC_KEY);
    chai.expect(result).to.equal(testData.SYS_ADDRESS);
  });

  it('Private Key from Mnemonic Seed Phrase',  () => {

    const result = SyscoinWeb3.keyStore.getPrivateKeyFromMnemonic(testData.SEED_PHRASE);
    chai.expect(result).to.equal(testData.SEED_PRIVATE_KEY);

    // console.log(SyscoinWeb3.keyStore.generateSeedPhrase());
  });
...
function loadJSON () {
  return {
    ...
    "SYS_ADDRESS": "SYS5WtmeekZLUS4vxCDhe9safyE6wFQ94EaczotN",
