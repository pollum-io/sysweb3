...
//NOTE: During recover account from seed phrase, detect ETH accounts - inform user of derivation path and compatibility?  ETH (reuse ETH accounts) or SYS (ledger support)
const CONSTANTS = {
  BIP_44_SYS_PATH: `m/44'/${SYS_COIN}'/0'/0`,
...
const DERIVATION_PATH_MAP = {
  [DERIVATION_PATH.SYS]: CONSTANTS.BIP_44_SYS_PATH,
...
export class KeyStore {
  ...
  getPrivateKeyFromMnemonic(mnemonic: string, derivationPath: DERIVATION_PATH = DERIVATION_PATH.SYS) {
    if (bip39.validateMnemonic(mnemonic)) {
      const seedBytes = bip39.mnemonicToSeedSync(mnemonic);
  ...
  getMasterKeyFromMnemonic(mnemonic: string, derivationPath: DERIVATION_PATH = DERIVATION_PATH.SYS): HDKey {
    if (bip39.validateMnemonic(mnemonic)) {
      const seedBytes = bip39.mnemonicToSeedSync(mnemonic);
  ...
  validatesysAddress(address: string) {
    ...
    const validPrefix = address.substr(0, 3) === 'SYS';
  ...
  getsysAddressFromPrivateKey(privateKeyHex: string) {
    return this.getsysAddressFromPublicKey(this.getPublicKeyFromPrivate(privateKeyHex));
  }
  ...
  getsysAddressFromPublicKey(publicKeyHex: string) {
