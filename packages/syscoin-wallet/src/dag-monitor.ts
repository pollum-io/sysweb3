import {crossPlatformDi} from '@syscoin/syscoin-core';
import {blockExplorerApi, globalSyscoinNetwork, loadBalancerApi, Transaction} from '@syscoin/syscoin-network';
import {Subject} from 'rxjs';
import {PendingTx} from '@syscoin/syscoin-network/types';
import {CbTransaction} from '@syscoin/syscoin-network/types/dto/cb-transaction';
...
export class syscoinMonitor {

  private memPoolChange$ = new Subject<syscoinWalletMonitorUpdate>();
  ...
  addToMemPoolMonitor (value: PendingTx | string) {
    ...
    const key =  `network-${globalSyscoinNetwork.getNetwork().id}-mempool`;
  ...
  async getLatestTransactions (address: string, limit?: number, searchAfter?: string): Promise<Transaction[]> {
    const cTxs = await globalSyscoinNetwork.blockExplorerApi.getTransactionsByAddress(address, limit, searchAfter);
  ...
  getMemPoolFromMonitor(address?: string): PendingTx[] {
    ...
    const txs: PendingTx[]  = this.cacheUtils.get(`network-${globalSyscoinNetwork.getNetwork().id}-mempool`) || [];
