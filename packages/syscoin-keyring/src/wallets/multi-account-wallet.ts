...
export class MultiAccountWallet implements IKeyringWallet {
  ...
  create(network: KeyringNetwork, mnemonic: string | BIP39_WORD_COUNT, label: string, numOfAccounts = 1) {
    ...
    this.deserialize({ secret: String(mnemonic), type: this.type, label, network, numOfAccounts })
  }
  ...
  deserialize(data: KeyringWalletSerialized) {
    ...
    if (this.network === KeyringNetwork.Syscoin) {
      this.supportedAssets.push(KeyringAssetType.SYS);
      bip44Path = BIP_44_PATHS.SYSCOIN_PATH;
    }
    else {
